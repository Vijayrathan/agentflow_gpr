...
#          - with gcc -fopenmp option implies -pthread
    ...
    def _find_gcc(bin_dir):
        """Find gcc executables (excluding gcc-ar, gcc-nm, gcc-ranlib) and sort by version."""
        candidates = glob.glob(os.path.join(bin_dir, 'gcc-[0-9]*'))
        # Filter out gcc-ar, gcc-nm, gcc-ranlib etc.
        candidates = [p for p in candidates if os.path.basename(p).startswith('gcc-') and os.path.basename(p).replace('gcc-', '').isdigit()]
        # Sort by version number
        candidates.sort(key=lambda p: int(os.path.basename(p).split('-')[-1]))
        return candidates

    def _gcc_version(gcc_path):
        """Extract the numeric version from a gcc path like /opt/homebrew/bin/gcc-15."""
        return os.path.basename(gcc_path).split('-')[-1]

    if 'Apple' in cpuID:
        gccpath = _find_gcc('/opt/homebrew/bin')
        if gccpath:
            gcc_name = os.path.basename(gccpath[-1])
            gcc_ver = _gcc_version(gccpath[-1])
            os.environ['CC'] = gcc_name
            rpath = '/opt/homebrew/opt/gcc/lib/gcc/' + gcc_ver + '/'
        else:
            raise Exception('Cannot find gcc in /opt/homebrew/bin. gprMax requires gcc to be installed - easily done through the Homebrew package manager (http://brew.sh). Note: gcc with OpenMP support is required.')
    else:
        gccpath = _find_gcc('/usr/local/bin')
        if gccpath:
            gcc_name = os.path.basename(gccpath[-1])
            gcc_ver = _gcc_version(gccpath[-1])
            os.environ['CC'] = gcc_name
            rpath = '/usr/local/opt/gcc/lib/gcc/' + gcc_ver + '/'
        else:
            raise Exception('Cannot find gcc in /usr/local/bin. gprMax requires gcc to be installed - easily done through the Homebrew package manager (http://brew.sh). Note: gcc with OpenMP support is required.')
